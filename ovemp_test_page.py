...
class OVEMPTestPage(ttk.Frame):
    ...
    def set_data(self, data):
        ...
        for side in sides:
            ...
            getattr(self, f"{prefix}_n10_latency_var").set(data.get(f"{side}N10波潜伏期 (毫秒)", ""))
            getattr(self, f"{prefix}_p15_latency_var").set(data.get(f"{side}P15波潜伏期 (毫秒)", ""))
            getattr(self, f"{prefix}_n10_p15_interval_var").set(data.get(f"{side}N10-P15波间期 (毫秒)", ""))
            getattr(self, f"{prefix}_n10_amplitude_var").set(data.get(f"{side}N10波振幅 (微伏)", ""))
            getattr(self, f"{prefix}_p15_amplitude_var").set(data.get(f"{side}P15波振幅 (微伏)", ""))
